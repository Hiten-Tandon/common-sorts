...
mod tests {
    ...
    fn string_radix_sort() {
        let mut vec: Vec<String> = (0..100_000)
            .map(|_| {
    ...
    fn f32_slice_radix_sort_test() {
        let mut vec: VecDeque<f32> = (0..SIZE)
            .map(|_| rand::random::<f32>() * rand::random::<i32>() as f32)
            .collect();
        let mut vec_clone = vec.clone();
        vec.make_contiguous().radix_sort();
    ...
    fn f64_slice_radix_sort_test() {
        let mut vec: VecDeque<f64> = (0..SIZE)
            .map(|_| rand::random::<f64>() * rand::random::<i64>() as f64)
            .collect();
        let mut vec_clone = vec.clone();
        vec.make_contiguous().radix_sort();
    ...
    fn string_slice_radix_sort() {
        let mut vec: VecDeque<String> = (0..100_000)
            .map(|_| {
                let iter = 0..=rand::thread_rng().gen_range(0..=100);
                iter.map(|_| rand::random::<char>()).collect()
            })
